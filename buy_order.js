...
async function swapExactTokensForWETH() {
  try {
    ...
    console.log(
      `buy_order.js: Starting Wallet WETH Balance: ${ethers.formatEther(
        initialWETHBalance
      )} WETH`
    );
    console.log(
      `buy_order.js: Starting Wallet USDT Balance: ${ethers.formatUnits(
        initialUSDTBalance,
        6
      )} USDT`
    );
    ...
    await (
      await USDTContract.approve(UNISWAP_ROUTER_ADDRESS, amountInUSDT)
    ).wait();
    ...
    console.log(
      `buy_order.js: Final WETH Balance: ${ethers.formatEther(
        finalWETHBalance
      )} WETH`
    );
    console.log(
      `buy_order.js: Final USDT Balance: ${ethers.formatUnits(
        finalUSDTBalance,
        6
      )} USDT`
    );
